...
--[[
klhtu.combat.submitgbokcast()
When you think you have just cast gbok, call this method to make sure the addon knows.

]]



me.addgbokthreat = function(target)
	me.event.hits = 1
	me.event.rage = 0
	...
	me.event.threat = numclass * 114;
	mod.table.mydata[mod.string.get("threatsource", "total")].threat = mod.table.mydata[mod.string.get("threatsource", "total")].threat + me.event.threat
	KLHTM_RequestRedraw("self")
	
end
-- Call this from a macro, to replace your normal sunder button
me.spellbookgbokindex = 1;
...
me.greaterblessingofkings = function(target)
	...
	-- 1) Check if the old position is fine
	if GetSpellName(me.spellbookgbokindex, "spell") ~= mod.string.get("spell", "greaterblessingofkings") then
		...
		-- 2) locate sunder armor in the spell book
		for i = 1, numspells do
			if GetSpellName(me.spellbookgbokindex + i, "spell") == mod.string.get("spell", "greaterblessingofkings") then
				me.spellbookgbokindex = me.spellbookgbokindex + i
				break
				
			elseif i == numspells then
				if mod.out.checktrace("warning", me, "greaterblessingofkings") then
					mod.out.printtrace("Can't find greaterblessingofkings in your spellbook!")
				end
				return -- can't find sunder
			end
		end
	...
	-- Now we've found sunder. Check the cooldown		
	if GetSpellCooldown(me.spellbookgbokindex, "spell") ~= 0 or (UnitMana("player") < 150) or not (UnitIsConnected(target)) or UnitIsDead(target) then 
		return
	end
	...
	-- Cast
	me.savedcastspellbyname1(mod.string.get("spell", "greaterblessingofkings"))
	me.addgbokthreat(target)
...
me.saveduseaction1 = UseAction

me.newuseaction1 = function(actionindex, x, y)
	
	-- Check Sunder
	if (mod.my.class == "paladin") and actionindex and (GetActionText(actionindex) == nil) and (GetActionTexture(actionindex) == "Interface\\Icons\\Spell_Magic_Greaterblessingofkings") then
		
		if GetActionCooldown(actionindex) > 0 and UnitMana("player") < 150 then
			if mod.out.checktrace("info", me, "greaterblessingofkings") then
				mod.out.printtrace("Preventing Sunder cast due to cooldown!")
			end
			return
			...
			me.greaterblessingofkings("target")
			-- At the bottom of this method the sunder will be cast
		end
   ...
   -- Call the original function
   me.saveduseaction1(actionindex, x, y)   

end

UseAction = me.newuseaction1


--[[
This code hooks CastSpellByName(), intercepts the user casting Sunder Armor, and runs me.sunder() instead.
]]
me.savedcastspellbyname1 = CastSpellByName

me.newcastspellbyname1 = function(name, onself)
	
	if string.find(name, mod.string.get("spell", "greaterblessingofkings")) then
		-- user wants to cast sunder. Run it through me.sunder() instead
		me.greaterblessingofkings("target")
		
	else
		me.savedcastspellbyname1(name, onself)
	end
	
end

CastSpellByName = me.newcastspellbyname1